...
def train_models(feature_folders, feature_type, model_dir, do_cv=True, processes=1):
    ...
    if feature_type in ('hills', 'wavelets'):
        load_function = wavelet_classification.load_csv
        file_pattern = "extract_features_for_segment.csv",
    elif feature_type == 'xcorr':
        load_function = correlation_convertion.load_and_pivot
        file_pattern = "*segment*.csv",
    else:
        raise ValueError("The feature type needs to be either 'wavelets', 'hills' or 'xcorr'")
    if do_cv:
...
def train_on_subject(subject_folder,
                     ...
                     processes=1):
    interictal = dataset.load_interictal_dataframes(subject_folder, load_function=load_function, file_pattern=file_pattern,
                                        processes=processes)
    preictal = dataset.load_preictal_dataframes(subject_folder, load_function=load_function, file_pattern=file_pattern,
                                        processes=processes)
    model_file = os.path.join(model_dir, fileutils.get_subject(subject_folder)) + '.pickle'
    model = seizure_modeling.train_model(interictal,
                                         preictal,
                                         method=method,
                                         processes=processes,
                                         no_crossvalidation=no_crossvalidation)
    if not no_crossvalidation:
...
def predict(feature_folders, feature_type, model_dir, submission_folder, processes=1):
    ...
    for subject in subject_folders:
        score = predict_on_subject(subject, model_dir, load_function, file_pattern=file_pattern, processes=processes)
        scores.append(score)

    submission_file = os.path.join(submission_folder, 'submission.csv')
    with open(submission_file, 'w') as submission_fp:
        submissions.write_scores(scores, output=submission_fp, do_normalize=True)


def predict_on_subject(subject_folder,
                       model_dir,
                       load_function,
                       file_pattern="*segment*.csv",
                       processes=1):
    test = dataset.load_test_dataframes(subject_folder,
                                        load_function=load_function,
                                        file_pattern=file_pattern,
                                        processes=processes)
    model_file = os.path.join(model_dir, fileutils.get_subject(subject_folder)) + '.pickle'
    with open(model_file, 'rb') as fp:
        model = pickle.load(fp, enconding='bytes')
        scores = write_scores(subject_folder, test, model)
        return scores
